...
if [[ ! -f $SENTINEL_FILE ]]; then
  ...
  # cmake is required by IsaacLab's egl_probe build — install via conda (no sudo needed)
  conda install -c conda-forge -y cmake
  # C/C++ compilers are needed; install via conda if not available on the system
  if ! command -v gcc &>/dev/null; then
    conda install -c conda-forge -y compilers
  fi
